...
public class SubAwardHomeAction extends SubAwardAction {
    ...
    /**
     * This method is used to handle the edit button action on an ACTIVE
     * Subaward. If no Pending version exists for the same subawardCode, a new
     * Subaward version is created. If a Pending version exists, the user is
     * prompted as to whether she would like to edit the Pending version.
     * Answering Yes results in that Pending version SubAwardDocument to be
     * opened. Answering No simply returns the user to the ACTIVE document
     * screen
     *
     * @param mapping
     * @param form
    ...
    /**
     * .
     * This method prepares the SubAwardForm with the document found via the
     * SubAward lookup Because the helper beans may have preserved a different
...
//      this.getBusinessObjectService().delete(subAwardFundingSource); // let save() do this
    ...
    /**
     * This method is for adding contacts This method is for @throws
     * Exception...
     *
     * @param mapping the ActionMapping
     * @param form the ActionForm
    ...
    public ActionForward addCloseouts(ActionMapping mapping,
        ...
        if (new SubAwardDocumentRule().processAddSubAwardCloseoutBusinessRules(subAwardCloseout)) {
            if (subAwardCloseout.getDateFollowup() == null) {
                subAwardCloseout.setDateFollowup(this.getSubAwardService().getCalculatedFollowupDate(
