...
public class IacucLocationNameValuesFinder extends UifKeyValuesFinderBase {
    ...
    /**
     * Constructs the list of Iacuc Location Names. Each entry in the list is a
     * &lt;key, value&gt; pair, where the "key" is the unique location id and
     * the "value" is the textual description that is viewed by a user. The list
     * is obtained from the IACUC_LOCATION_NAME database table via the
     * "KeyValuesService".
     *
    ...
    public List<KeyValue> getKeyValues() {
        ...
        for (Iterator<IacucLocationName> iter = iacucLocationNames.iterator(); iter.hasNext();) {
            IacucLocationName iacucLocationName = iter.next();
