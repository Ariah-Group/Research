...
/**
 * The Proposal Copy Service creates a new Proposal Development Document based
 * upon a current document and criteria specified by a user.
 *
 ...
 * Document. By having a new document, its initiator and timestamp are set
 * correctly and all workflow information is in its initial state, e.g. there
 * are no adhoc routes.
 * </li>
 * <li>Most of the properties declared within the ProposalDevelopmentDocument
 * are copied from the original document to the new document. Some properties,
 * such as <b>proposalNumber</b> are filtered out, i.e. they are not copied
 * during this phase.<br><br>
 *
 * The copying of the properties is done via reflection. Any property that has a
 * setter and a getter method is copied. Property values that are
 * <i>Serializable</i> are copied using
 * <b>ObjectUtils.deepCopy()</b>. Copying by reflection allows the copy service
 * to automatically copy any new properties that are added to the document
 * without the requiring the programmer to modify this copy service.
 * </li>
 * <li>The Document Overview properties are copied. These are the description,
 * explanation, and organization doc number fields on the document. Since they
 * belong to the base document class, they are not copied in step 2.
 * </li>
 * <li>The LeadUnit is set according to a user's selection.
 * </li>
 * <li>If the attachments are included, they are then copied. This includes
 * copying the contents of the attachment.
 * </li>
 ...
 * </ul>
 *
 * The <b>ProposalCopyCriteria</b> contains the user specified criteria, e.g.
 * whether or not to copy attachments, etc.
 *
...
public class ProposalCopyServiceImpl implements ProposalCopyService {
    ...
    /**
     * The set of Proposal Development Document properties that must not be
    ...
    /**
     * Each property in the document that can be copied is represented by its
...
//          Copy over the budget(s) if required by the user.  newDoc must be saved so we know proposal number.
     ...
     * document in order to obtain a proposal number.
     *
     * @param srcDoc
     * @param destDoc
     */
    ...
    /**
     * Copies the source proposal development document to the destination
     * document.
     *
    ...
    /**
     * Copies over the "normal" Proposal Development Document properties. Only
     * the properties declared within the ProposalDevelopmentDocument class are
     * copied. Properties from parent classes are not copied.
     *
     * @param src the source proposal development document, i.e. the original.
     * @param dest the destination proposal development document, i.e. the new
     * document.
     * @throws Exception if the copy fails for any reason.
     */
    ...
    /**
     * The document next values must be the same in the new version as in the
     * old document. Note that the next document values must be assigned the
     * document number of the new version.
     *
     * @param oldDoc
     * @param newDoc
     */
     ...
     * properties belong to a parent class and thus they were not copied over in
     * step 2.
     *
     * @param src the source proposal development document, i.e. the original.
     * @param dest the destination proposal development document, i.e. the new
    ...
    /**
     * Get the list of DevelopmentProposal properties that can be copied. A
     * property can only be copied if it meets the following criteria.
     * <ul>
    ...
    /**
     * Get the name of a property. The method must be a setter method. The
     * property name is computed by removing the "set" from the method name.
     *
    ...
    /**
     * If the Lead Unit has changed in the previous {@link Document}, then this
     * method corrects related properties {@link Organization} and
     * {@link ProposalSite} instances
     *
     ...
     * @param list I assume this is the list of objects that have already been
     * processed.
     */
    ...
    /**
     * Is the given method a getter method for a property? Must conform to the
     * following:
     * <ol>
    ...
    /**
     * This method sets all congressional district ids to null, so new ids get
     * assigned to them.
     *
     * @param doc
     */
    ...
    /**
     * Copy the Attachments (proposal, personal, and institutional) to the new
     * document. Does this by loading the actual attachments (since they are
     * left out of the object graph under normal conditions, then copies the
     * attachments (ProposalPersonBiographies, Narratives, and
     * InstituteAttachments).
     *
     * @param src the source proposal development document, i.e. the original.
     * @param dest the destination proposal development document, i.e. the new
     ...
     *
     * @param doc the proposal development document to load attachment contents
     * into.
     */
    protected void loadAttachmentContents(ProposalDevelopmentDocument doc) {
        ...
        for (Narrative narrative : narratives) {
            loadAttachmentContent(narrative);
            /**
             * The getNarratives collection contains both personal and
             * instiutional attachments for persisting and displaying. Am not
             * sure why it was implemented this way, but do to time constraints
             * significant refactor isn't ideal.
             */
    ...
    /**
     * Load the attachment content for a specific personal attachment from the
     * database.
     *
    ...
    /**
     * Recurse through all of the BOs and if a BO has a specific property, set
