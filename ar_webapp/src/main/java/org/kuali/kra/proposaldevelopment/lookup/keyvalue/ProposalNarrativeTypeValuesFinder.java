...
/**
 * Finds the available set of supported Narrative Types. See the method
 * <code>getKeyValues()</code> for a full description.
 *
 * @author KRADEV team
 *
 * Rewrite of class to implement filterable KeyValue list.
 *
 * A complete rewrite had to be done because super class was not editable, but
 * the patterns used here should be incorporated into super class. This class
 * would then be refactored to again rely more on super class behavior. Super
 * class rewrite should include isolating behaviors relying on infrastructure
 * into protected methods to allow unit tests to override with mocks and stub
 * out database and service lookups.
 */
public class ProposalNarrativeTypeValuesFinder extends FormViewAwareUifKeyValuesFinderBase {
    ...
    protected Collection<NarrativeType> loadAllNarrativeTypes() {
        ...
        if (isS2sCandidate(developmentProposal)) {
            ...
            String proposalNarrativeTypeGroup = this.getParameterService().getParameterValueAsString(
                    Constants.MODULE_NAMESPACE_PROPOSAL_DEVELOPMENT,
                    Constants.PARAMETER_COMPONENT_DOCUMENT,
                    Constants.PROPOSAL_NARRATIVE_TYPE_GROUP);
