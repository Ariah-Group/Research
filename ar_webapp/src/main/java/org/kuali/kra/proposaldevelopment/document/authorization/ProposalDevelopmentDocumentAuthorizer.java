...
/**
 * The Proposal Development Document Authorizer. Primarily responsible for
 * determining if a user has permission to create/modify/view proposals.
 *
...
public class ProposalDevelopmentDocumentAuthorizer extends KcTransactionalDocumentAuthorizerBase {
    ...
    public Set<String> getEditModes(Document document, Person user, Set<String> currentEditModes) {
        ...
        boolean proposalLockedByAdmin = proposalDoc.getDevelopmentProposal().isLocked();

        if (proposalLockedByAdmin) {
            for (ProposalPerson person : proposalDoc.getDevelopmentProposal().getProposalPersons()) {
                if (person.getUserName().equals(user.getPrincipalName())) {
                    System.out.println("Removing Proposal Dev Edit Mode modifyProposal for user : " + user.getPrincipalName());
                    editModes.remove(KraAuthorizationConstants.ProposalEditMode.MODIFY_PROPOSAL);
                    editModes.add(AuthorizationConstants.EditMode.VIEW_ONLY);
                    break;
                }
            }
    ...
    /**
     * Set the permissions to be used during the creation of the web pages. The
     * JSP files can access the editModeMap (editingMode) to determine what to
     * display to the user. For example, a JSP file may contain the following:
     *
     ...
     * </kra:section>
     *
     * In the above example, the contents are only rendered if the user is
     * allowed to modify the proposal. Note that permissions are always
     * signified as either TRUE or FALSE.
     *
    ...
    /**
     * Does the user have permission to create a proposal. Use the Unit
     * Authorization Service to determine if the user has the CREATE_PROPOSAL
     * permission in any unit.
     *
     * @param user the user
     * @return true if the user has the CREATE_PROPOSAL permission in at least
     * one unit; otherwise false
     */
