...
public class KeyPersonnelCertificationRule extends ResearchDocumentRuleBase implements DocumentAuditRule {
    ...
    private boolean hasCertification(ProposalPerson person) {
        ...
        if (personRole.getRoleCode().equals(Constants.PRINCIPAL_INVESTIGATOR_ROLE)
                || (personRole.getRoleCode().equals(Constants.CO_INVESTIGATOR_ROLE)
                && StringUtils.isNotBlank(person.getOptInCertificationStatus())
    ...
    /**
     * Yes/No questions have to be submitted to Grants.gov on document route. If
     * the submitter has not completed the certifications, errors should be
     * displayed in audit mode.<br/>
     * <br/>
     * This method differs from
     * <code>{@link #validateKeyPersonCertification(ProposalDevelopmentDocument)}</code>
     * that it refers to a specific person. If any one of the Yes/No Questions
     * is not completed, then this check will fail.
     *
     *
