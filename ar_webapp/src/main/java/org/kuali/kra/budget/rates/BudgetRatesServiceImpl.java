...
public class BudgetRatesServiceImpl<T extends BudgetParent> implements BudgetRatesService<T> {
    ...
    /**
     * reset budget rates for a panel each panel is based on rate class type
     *
     */
    @Override
...
//        ProposalDevelopmentDocument proposal = (ProposalDevelopmentDocument)budgetDocument.getParentDocument();
    ...
    protected Date getRateEffectiveStartDate(Budget budget, AbstractInstituteRate rate, Date personEffectiveDate) {
        ...
        if (rate.getRateClass() == null) {
            LOG.error("Rate " + rate.getRateKeyAsString() + ", Fiscal Year " + rate.getFiscalYear() + ", Unit" + rate.getUnitNumber() + " contains an EMPTY RATE CLASS CODE");
            return effectiveDate;
        }

        if (rate.getRateClass().getRateClassType() == null) {
            LOG.error("Rate Class " + rate.getRateClassCode() + ", Fiscal Year " + rate.getFiscalYear() + ", Unit" + rate.getUnitNumber() + " contains an EMPTY RATE CLASS TYPE CODE");
            return effectiveDate;
        }
    ...
    protected String getLocationFlagAsString(boolean onOffCampusFlag) {
        return onOffCampusFlag ? Constants.ON_CAMPUS_FLAG : Constants.OFF_CAMPUS_FLAG;
    }
    ...
    /**
     * Searches for persisted {@link RateClass} instances based on the given
     * <code>rateClassType</code>. Uses the {@link BusinessObjectService} to
     * grab appropriate {@link RateClass} instances since {@link RateClass} is a
     * {@link BusinessObject}
     *
    ...
    /**
     * Retrieves {@link RateClass} instances as a {@link Map} keyed from the
     * <code>rateTypeCode</code>. This makes it easy for classes (particularly
     * in the UI) to grab {@link RateClass} information via rateTypeCode
     *
     * @param rateClassType to use for {@link RateClass} instances to be
     * retrieved
     * @return a {@link Map} keyed on rateTypeCode containing {@link RateClass}
     * instances
     */
    @Override
