...
public class InstitutionalProposalCreditSplitBean implements Serializable {
    ...
    /**
     * This constructor should only be called when no InstitutionalProposalForm
     * is not available The InstitutionalProposalForm reference is stable during
     * a session, but InstitutionalProposalDocument is not However, in rule
     * processing, a Form may not be available, especially if from a unit test.
     *
     * In that case, this constructor should be used. The
     * recalculateCreditSplit(InstitutionalProposalDocument) method should be
     * called, passing in an InstitutionalProposalDocument
     */
    ...
    /**
     * This method prepares all project personnel, and their units, with empty
     * credit splits for any InvestigatorCreditTypes that aren't already
     * represented.
     *
     * @return
     */
    ...
    /**
     * This method fetches the InstitutionalProposal Credit Splits enabled
     * system parm
     *
     * @param parmName
     * @return
    ...
    protected String fetchParameterValue(String parmName) {
        return this.getParameterService().getParameterValueAsString(Constants.MODULE_NAMESPACE_INSTITUTIONAL_PROPOSAL,
                Constants.PARAMETER_COMPONENT_DOCUMENT, parmName);
    }
    ...
    /**
     * Find the Institutional Proposal, first by looking to a form bean, if any.
     * If no form bean exists, check if the document exists. If neither exists,
     * returned Institutional Proposal is null
     *
     * @return
     */
    ...
    /**
     * A keyPerson may have no associated unit. To satisfy the validation
     * checks, we apply this workaround to set the unit credit split type totals
     * to 100.00
     *
     * @param personUnitCreditTotals
     */
