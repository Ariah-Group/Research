...
public class AwardSyncServiceImpl implements AwardSyncService {
    ...
    /**
     * @see
     * org.kuali.kra.award.awardhierarchy.sync.service.AwardSyncService#validateHierarchyChanges(org.kuali.kra.award.home.Award)
     */
    ...
    /**
     * @see
     * org.kuali.kra.award.awardhierarchy.sync.service.AwardSyncService#applyAwardSyncChangesToHierarchy(org.kuali.kra.award.home.Award)
     */
    ...
    /**
     * @see
     * org.kuali.kra.award.awardhierarchy.sync.service.AwardSyncService#getAwardLockingHierarchyForSync(org.kuali.kra.award.document.AwardDocument,
     * java.lang.String)
     */
    ...
    /**
     * Run validation or full sync depending on syncType on the award hierarchy
     * under award.
     *
     * @param award
     * @param syncType
    ...
    /**
     * Clears sync status from parent award as long as it has an award
     * number(not the parent status), and it is not yet complete.
     *
     * @param award
     * @param runnables
     */
    ...
    /**
     * Sets the parent award status for the sync. This is done by creating and
     * updating a sync status that has no award.
     *
     * @param parentAward
     * @param success
     * @param newStatus
    ...
    /**
     * Finds the parents award status log entry, the one without an award
     * listed.
     *
     * @param parentAward
     * @return
    ...
    /**
     * Check and validate the status of the award. Must have an active version
     * and there must not be a pending version.
     *
     * @param awardNumber
     * @param awardStatus
            ...
            if (!awardStatus.isSyncComplete()) {
                ...
                if (oldAward != null) {
                    if (getAwardSyncSelectorService().isAwardInvolvedInSync(oldAward, changes)) {
                        ...
                        result &= applyAndValidateChanges(awardDocument.getAward(), awardStatus, changes);
                        if (result) {
    ...
    /**
     * Apply the list of changes against the award and then validate the
     * changes.
     *
     * @param award
     * @param awardStatus
    ...
    /**
     * Finalize the award sync. If running validation this only requires setting
     * the AwardSyncStatus to final. If running full sync the award is saved,
     * FYI list generated and award blanket approved.
     *
     * @param awardDocument
     * @param awardStatus
    ...
    /**
     * Attempts to load the award document for the specified award. This ensures
     * that the workflow document is not null and can be used for validation.
     *
     * @param awardStatus
     * @param award
     * @param errorMessage
    ...
    /**
     * Set the status to failure and set the status to the message and add the
     * log message.
     *
     * @param awardStatus
     * @param statusMessage
    ...
    /**
     * Ensure that the person specified by principalId has modify permission on
     * the award specified.
     *
     * @param awardStatus
     * @param award
    ...
    /**
     * Run the {@link AwardDocumentRule#processSaveDocument} and
     * {@link AwardDocumentRule#processRunAuditBusinessRules} against award. Add
     * all messages generated from running the rules to logList as
     * {@link AwardSyncLog}. Return false if any of the error keys generated by
     * the rules are not in the ignoredMessageKeys list.
     *
     * @param award
     * @param logs
     * @return
    ...
    /**
     * Get the syncHelper from the syncHelpers for the given className. If the
     * className is a full qualified class name, only the class name itself is
     * used.
     *
     * @param className
     * @return
    ...
    /**
     *
     * Collects all prinicipalsToNotify from {@link #RunSyncOnAward} runnables
     * in the list of runnables.
     *
     * @param runnables
     * @return
     */
    ...
    /**
     * Create new version of oldAward and prepare document for sync by setting
     * document description, transaction type, etc.
     *
     * @param parentAward
     * @param oldAward
     * @param logList
    ...
    /**
     * Run the runnable in a separate transaction, commiting when finished or
     * rolling back if an exception is generated.
     *
     * @param runnable
     */
    ...
    /**
     *
     * Run {@link AwardSyncServiceImpl#applyChanges} through the use of
     * {@link #RunSyncOnAward}.
     *
     * @param parentAward
    ...
    /**
     *
     * Send ad hoc requests in a thread. This is used to make sure the requests
     * are processed before the document moves to the next step in workflow and
     * then the requests get ignored.
     *
     * @param document
     * @param annotation
     * @param recipients
    ...
    /**
     * Parent for thread based runnables related to award sync to provide a
     * finished flag that supports ensuring all threads are finished.
     */
    ...
    /**
     *
     * Runnable that calls {@link AwardSyncServiceImpl#applyChanges} in a
     * separate thread.
     */
