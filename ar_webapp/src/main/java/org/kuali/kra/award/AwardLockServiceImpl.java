...
public class AwardLockServiceImpl extends PessimisticLockServiceImpl implements ProposalLockService {
    ...
    /**
     * This method is used to check if the given parameters warrant a new lock
     * to be created for the given user. This method utilizes the
     * {@link #isEntryEditMode(java.util.Map.Entry)} method.
     *
     * @param document - document to verify lock creation against
     * @param editMode - edit modes list to check for 'entry type' edit modes
     * @param user - user the lock will be 'owned' by
     * @return true if the given edit mode map has at least one 'entry type'
    ...
    /**
     * This method is used to check if the given {@link Map.Entry} is an 'entry
     * type' edit mode and that the value is set to signify that this user has
     * that edit mode available to them
     *
     * @param entry - the {@link Map.Entry} object that contains an edit mode
     * such as the ones returned but
     *
     * @return true if the given entry has a key signifying an 'entry type' edit
     * mode and the value is equal to {@link #EDIT_MODE_DEFAULT_TRUE_VALUE}...
     * false if not
     */
    @SuppressWarnings("unchecked")
    @Override
