...
public class AwardApprovedEquipmentRuleImpl extends ResearchDocumentRuleBase
    ...
    /**
     * @see
     * org.kuali.kra.award.paymentreports.specialapproval.approvedequipment.AwardApprovedEquipmentRule#processAwardApprovedEquipmentBusinessRules(org.kuali.kra.award.paymentreports.specialapproval.approvedequipment.AwardApprovedEquipmentRuleEvent)
     */
    ...
    /**
     * An equipment item is unique if no other matching items are in the
     * collection To know if this is a new add or an edit of an existing
     * equipment item, we check the identifier for nullity. If null, this is an
     * add; otherwise, it's an update If an update, then we expect to find one
     * match in the collection (itself). If an add, we expect to find no matches
     * in the collection
     *
