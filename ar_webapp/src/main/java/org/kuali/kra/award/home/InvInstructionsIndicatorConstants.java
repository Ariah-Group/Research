 ...
 *
 * Optional: The user can fill out the invoice instructions, but it is not
 * required. Mandatory: The user must fill out the invoice instructions. Blank:
 * The user must not fill it out, and the rendered field for instructions shall
 * be disabled.
 */
public enum InvInstructionsIndicatorConstants {
    ...
    public static InvInstructionsIndicatorConstants getByCode(String code) {
        for (InvInstructionsIndicatorConstants cont : InvInstructionsIndicatorConstants.values()) {
            if (StringUtils.equals(cont.code, code)) {
                return cont;
            }
        }
        return null;
