...
public class ProtocolOnlineReviewAction extends ProtocolAction implements AuditModeAction {
    ...
    private boolean validateCreateNewProtocolOnlineReview(ProtocolForm protocolForm) {
        ...
        if (protocolForm.getOnlineReviewsActionHelper().getNewReviewDateRequested() != null && protocolForm.getOnlineReviewsActionHelper().getNewReviewDateDue() != null) {
            if ((DateUtils.isSameDay(protocolForm.getOnlineReviewsActionHelper().getNewReviewDateDue(),
                ...
                //no-op
            } else {   //dates are not the same or due date is before requested date
                valid = false;
     ...
     * using: (actionMethodToCall).(onlineReviewDocumentNumber).anchor in the
     * name attributes of the online review form buttons. Where
     *
     ...
     *
     * @param parameterName The parameter being decoded, usually retrieved from
     * the request parameters as KNSConstants.METHOD_TO_CALL_ATTRIBUTE.
     * @param actionMethodToCall The methodToCall ( function name in the action
     * being executed. ).
     *
     * @return
     */
    ...
    /**
     *
     * This method is to render protocol review page. It is redirected to by the
     * protocol online review redirect action when the edit link is clicked on
     * in the action list.
     *
     * @param mapping
     * @param form
        ...
        if (question == null) {
            ...
            if ((DOCUMENT_DELETE_QUESTION.equals(question)) && ConfirmationQuestion.NO.equals(buttonClicked)) {
                ...
                if (!this.applyRules(new DeleteProtocolOnlineReviewEvent(prDoc, reason, deleteNoteText, noteTextMaxLength))) {
                    ...
                    return this.performQuestionWithInputAgainBecauseOfErrors(mapping, form, request, response, DOCUMENT_DELETE_QUESTION, "Are you sure you want to delete this document?", KRADConstants.CONFIRMATION_QUESTION, callerString, "", reason, ERROR_DOCUMENT_DELETE_REASON_REQUIRED, KRADConstants.QUESTION_REASON_ATTRIBUTE_NAME, Integer.toString(reasonLimit));
                }
