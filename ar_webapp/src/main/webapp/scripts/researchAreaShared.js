...
$j("#add0")
        .click(function () {
            ...
            if (trNode.children('td:eq(1)').children('input:eq(0)').val() == "") {
                alert("must enter research area code");
            } else if (trNode.children('td:eq(2)').children('input:eq(0)').val() == "") {
                alert("must enter research area");
            } else {
                var raCode = trNode.children('td:eq(1)').children('input:eq(0)').val();
                var raExist = researchAreaExistsOnServer(raCode, deletedNodes)
                if (raExist == 'false'
                        && newNodes != ";"
                        && newNodes.indexOf(";"
                                + trNode.children('td:eq(1)').children('input:eq(0)').val() + ";") > -1) {
                    raExist = 'true';
                }
                ...
                if (raExist == 'false') {
                    ...
                    var item_text = trNode.children('td:eq(1)').children('input:eq(0)').val()
                            + " : "
                            + trNode.children('td:eq(2)').children('input:eq(0)').val();
                    
                    var listitem = setupListItem(trNode
                            .children('td:eq(1)').children('input:eq(0)').val(), trNode.children('td:eq(2)')
                            .children('input:eq(0)').val(), trNode.children('td:eq(3)').children('input:eq(0):checked').val());
                    ...
                    var inputtag = $j('<input type="hidden"></input>').attr("id", loadedId);
                    ...
                    newNodes = newNodes
                            + trNode.children('td:eq(1)').children('input:eq(0)').val() + ";";
                    // record changes
                    raChanges.create(icur,
                            trNode.children('td:eq(1)').children('input:eq(0)').val(),
                            '000001',
                            trNode.children('td:eq(2)').children('input:eq(0)').val(),
                            trNode.children('td:eq(3)').children('input:eq(0):checked').val());

                    // display add message
                    $j("#headermsg").html("Research Area Code " + trNode.children('td:eq(1)').children('input:eq(0)').val() + " added");
                    ...
                    alert("Research Area Code " + trNode.children('td:eq(1)').children('input:eq(0)').val() + " already exist");
                }
            }
            ;
...
function getEditRow(name, id) {
    ...
    tdTag1.click(function () {
        if ($j('#checkActive' + idx).prop("checked")) {
            if ((ulTagId == 'researcharea') || ($j('#checkActive' + ulTagId.substring(2)).prop("checked")) || ($j("#activeflag" + ulTagId.substring(2)).val().trim() == 'true')) {
                    ...
                    function () {
                        ...
                        if (trNode.children('td:eq(1)').children('input:eq(0)')
                            ...
                            if (raExist == 'false') {
                                ...
                                var item_text = trNode.children('td:eq(1)').children('input:eq(0)').val()
                                        + " : " + trNode.children('td:eq(2)').children('input:eq(0)').val();
                                var listitem = setupListItem(trNode.children('td:eq(1)').children('input:eq(0)').val(),
                                        trNode.children('td:eq(2)').children('input:eq(0)').val(),
                                        trNode.children('td:eq(3)').children('input:eq(0):checked').val());
                                ...
                                newNodes = newNodes + trNode.children('td:eq(1)').children('input:eq(0)').val() + ";";
...
function setupListItem(code, name, activeflag) {
    ...
    $j(tag).click(
            function () {
                $j(".hierarchydetail:not(#" + divId + ")").slideUp(300);
                if ($j(this).siblings('div:eq(1)').children('table:eq(0)').size() == 0) {
                    
                    var idx = $j(this).attr("id").substring(11);
                    tableTag(code + " : " + name, "item" + idx).appendTo($j("#listcontent" + idx));
        ...
        $j.ajax({
            ...
            success: function (xml) {
                ...
                if (liNode.children('ul').size() == 0) {
                    ulTag = $j('<ul class="filetree"></ul>').attr("id", "ul" + icur);
                } else {
                    ulTag = ulNode;
                }
                ...
                $j(xml)
                        ...
                        .each(
                                function () {
                                            ...
                                            .click(function () {
                                                ...
                                                if ($j(this)
                                                    ...
                                                    tableTag(
                                                            ...
                                                            "item"
                                                            + idx)
                                                            .appendTo($j("#listcontent" + idx));
                                                    
                                                    if ($j("#listcontent" + idx).is(":hidden")) {
                                                        $j("#listcontent" + idx).show();
                                                    }
                                                } else {
                                                    $j("#listcontent"+ idx).slideToggle(300);
                                    ...
                                    var listitem = $j('<li class="closed"></li>').attr("id", id).html(tag);
...
function getResearchAreaCode(node) {
    return $j("#racode" + node.attr("id").substring(4)).val();
}
...
function isActive(node) {
    if ($j("#activeflag" + node.attr("id").substring(4)).val().trim() == 'true') {
        return true;
    } else {
        return false;
    }
