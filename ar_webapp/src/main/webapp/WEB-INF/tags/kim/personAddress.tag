    ...
    <table cellpadding="0" cellspacing="0" summary="">
        <tr>
          ...
          /><kim:cell inquiry="${inquiry}" isLabel="true" textAlign="center" attributeEntry="${docAddressAttributes.active}" noColon="true" /> 
            <c:if test="${not inquiry and canModify}"><kul:htmlAttributeHeaderCell literalLabel="Actions" scope="col"/></c:if>
        </tr>
        <c:if test="${not inquiry and not readOnlyEntity and canModify}">
             <tr>
                <th class="infoline"><c:out value="Add:" /></th>
                <td>
                    <div align="center"><kul:htmlControlAttribute property="newAddress.addressTypeCode" attributeEntry="${docAddressAttributes.addressTypeCode}" readOnly="${readOnlyEntity}" /></div>
                </td>
                <td>
                    <div align="center"><kul:htmlControlAttribute property="newAddress.line1" attributeEntry="${docAddressAttributes.line1}" readOnly="${readOnlyEntity}" /></div>
                </td>
                <td>
                    <div align="center"><kul:htmlControlAttribute property="newAddress.line2" attributeEntry="${docAddressAttributes.line2}" readOnly="${readOnlyEntity}" /></div>
                </td>
                <td>
                    <div align="center"><kul:htmlControlAttribute property="newAddress.line3" attributeEntry="${docAddressAttributes.line3}" readOnly="${readOnlyEntity}" /></div>
                </td>
                <td>
                    <div align="center"><kul:htmlControlAttribute property="newAddress.city" attributeEntry="${docAddressAttributes.city}" readOnly="${readOnlyEntity}" /></div>
                </td>
                <td>
                    <div align="center"><kul:htmlControlAttribute property="newAddress.stateProvinceCode" attributeEntry="${docAddressAttributes.stateProvinceCode}" styleClass="fixed-size-200-select" readOnly="${readOnlyEntity}" /></div>
                </td>
                <td>
                    <div align="center"><kul:htmlControlAttribute property="newAddress.postalCode" attributeEntry="${docAddressAttributes.postalCode}" readOnly="${readOnlyEntity}" /></div>
                </td>
                <td>
                    <div align="center"><kul:htmlControlAttribute property="newAddress.countryCode" attributeEntry="${docAddressAttributes.countryCode}" styleClass="fixed-size-200-select" readOnly="${readOnlyEntity}" /></div>
                </td>
                <td>
                    <div align="center"><kul:htmlControlAttribute property="newAddress.dflt" attributeEntry="${docAddressAttributes.dflt}" readOnly="${readOnlyEntity}" /></div>
                </td>
                <td>
                    <div align="center"><kul:htmlControlAttribute property="newAddress.active" attributeEntry="${docAddressAttributes.active}" readOnly="${readOnlyEntity}" /></div>
                </td>
                <td>
                    <div align="center"><html:image property="methodToCall.addAddress.anchor${tabKey}" src='${ConfigProperties.kr.externalizable.images.url}tinybutton-add1.gif' styleClass="tinybutton"/></div>
                </td>
            </tr>
        </c:if>
        <c:forEach var="address" items="${KualiForm.document.addrs}" varStatus="status">
            <c:if test="${!(inquiry and readOnlyEntity and kfunc:isHiddenKimObjectType(address.addressType.code, 'kim.hide.PersonDocumentAddress.type'))}">
              <tr>
                <th class="infoline"><c:out value="${status.index+1}" /></th>
