...
/><c:if test="${!empty index}"><c:set var="degreeIndexHeader" value="${index + 1}" 
...
<td class="${tdClass}"><div align=left><span class="copy"><kul:htmlControlAttribute property="${degreeCodeProperty}" attributeEntry="${degreeCodeAttribute}" readOnly="${readOnly}" /></span></div><span class="fineprint"></span> </td>
...
<td class="${tdClass}"><kul:htmlControlAttribute property="${degree}.graduationYear" attributeEntry="${personDegreeAttributes.graduationYear}" readOnly="${readOnly}" /></td>
<td class="${tdClass}"><div align=left><kul:htmlControlAttribute property="${degree}.school" attributeEntry="${personDegreeAttributes.school}" readOnly="${readOnly}" /></div><span class="fineprint"></span> </td>                   
 <td class="${tdClass}"><div align=center>&nbsp;<kra:section permission="modifyProposal">
 <c:if test="${ not isParent }"><html:image property="methodToCall.${actionMethod}" src="${actionImage}" title="${actionTitle}" alt="${actionTitle}" styleClass="tinybutton ${actionStyle}"/></c:if>
 </kra:section>
 </div>
  </td>
 </tr>