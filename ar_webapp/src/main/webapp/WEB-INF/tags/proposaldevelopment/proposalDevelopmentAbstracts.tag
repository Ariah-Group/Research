...
<c:set var="defaultAbstractType" value="${KualiForm.defaultAbstractType}" />
            ...
            <kra:section permission="modifyProposal">   
                <tr class="addline"> 
                    ...
                    <td align="left" valign="middle">
                        <html:select property="newProposalAbstract.abstractTypeCode" tabindex="0" >                                              
                            <c:forEach var="option" items="${krafn:getOptionList('org.kuali.kra.proposaldevelopment.lookup.keyvalue.AbstractTypeValuesFinder', paramMap)}">
                        <html type="hidden" name="hidden${option.key}" value="def=${defaultAbstractType}, val=${option.key}" />
                        <c:choose>
                            <c:when test="${defaultAbstractType == option.key}">
