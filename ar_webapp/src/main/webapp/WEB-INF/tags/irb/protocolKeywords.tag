...
%><c:set var="protocolKeywordAttributes" value="${DataDictionary.IrbProtocolKeyword.attributes}" 
   	...
   	<h3><span class="subhead-left">Keywords</span></h3>
	    ...
	    <c:forEach var="protocolKeywords" items="${KualiForm.document.protocol.keywords}" varStatus="status">		
              ...
              <tr>
                ...
                <c:if test="${!readOnly}">
	                <html:image property="methodToCall.selectAllScienceKeyword.anchor${tabKey}" src="${ConfigProperties.kra.externalizable.images.url}tinybutton-selectall.gif" title="Select All" alt="Select All" styleClass="tinybutton" onclick="javascript: selectAllIrbProtocolKeywords(document);return false" />    
