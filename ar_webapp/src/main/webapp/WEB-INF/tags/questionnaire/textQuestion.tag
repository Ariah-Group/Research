...
<c:choose>
	...
	<c:otherwise>
		...
		<c:if test="${answerLength > 80}"><c:set var="fieldSize" value="80" /></c:if>
		<input type="text" class="Qanswer answer questionnaireAnswer" id="${questionFieldName}" name="${questionFieldName}" maxlength="${question.answerMaxLength}" size="${fieldSize}" value="${bean.answerHeaders[answerHeaderIndex].answers[questionIndex].answer}" />
	</c:otherwise>
</c:choose>
