...
--%>
<%@ tag import="java.util.Map, org.kuali.rice.kns.util.ActionFormUtilMap, java.util.ArrayList"
%><%@ include file="/kr/WEB-INF/jsp/tldHeader.jsp"
%><%@ attribute name="property" required="true" description="The property being rendered."
%><%@ attribute name="propertyId" required="false" description="The value of the ID element to be set. If blank, defaults to value of property attribute."
%><%@ attribute name="textStyle" required="false" description="The style attribute."
...
/><c:if test="${empty readOnly}"><c:set var="readOnly" value="false"/>
</c:if><c:if test="${!empty attributeEntry.attributeSecurityMask && attributeEntry.attributeSecurityMask == true  }">
...
<c:if test="${!displayMask && !empty attributeEntry.attributeSecurityPartialMask && attributeEntry.attributeSecurityPartialMask == true  }">
	<c:set var="className" value ="${attributeEntry.fullClassName}" /><c:set var="fieldName" value ="${attributeEntry.name}" />
    <c:set var="displayMask" value="${kfunc:canPartiallyUnmaskField(className, fieldName,KualiForm)? 'false' : 'true'}" />
	<c:set var="readOnly" value="${displayMask || readOnly}"/>
	<c:if test="${displayMask}"><c:set var="displayMaskValue" value="${kfunc:getPartiallyMaskedValue(className, fieldName, KualiForm, property)}" /></c:if>
</c:if>
...
<c:set var="accessibleTitle" value="${Constants.REQUIRED_FIELD_SYMBOL} ${accessibleTitle}"/>
  ...
  <c:if test="${!(empty accessibilityHint)}"><c:set var="accessibleTitle" value="${accessibleTitle} ${accessibilityHint}"/></c:if>
<kul:checkErrors keyMatch="${property}" auditMatch="${property}"/>
<c:set var="disableField" value="false" />
<c:if test="${disabled}"><c:set var="disableField" value="true" /></c:if>
<c:if test="${empty styleClass}"><c:set var="styleClass" value=""/></c:if>
<c:choose>
  <c:when test="${!empty tabindexOverride}"><c:set var="tabindex" value="${tabindexOverride}"/></c:when>
  <c:otherwise>
    <c:set var="tabindex" value="${KualiForm.currentTabIndex}"/><c:set var="dummyIncrementVar" value="${kfunc:incrementTabIndex(KualiForm, tabKey)}" />
  </c:otherwise>
</c:choose>
<c:if test="${readOnly}">
   <c:choose>
     <c:when test="${readOnlyBody}"><jsp:doBody/></c:when>
     <c:otherwise>
        <c:choose>
		<c:when test="${displayMask}" >${displayMaskValue}</c:when>
         <c:otherwise>
         <logic:empty name="KualiForm" property="${property}">&nbsp;</logic:empty>
...
<c:if test="${!readOnly}">
  <c:choose>
    ...
    <c:when test="${attributeEntry.control.select == true}">
            <c:set var="finderClass" value="${fn:replace(attributeEntry.control.valuesFinder,'.','|')}"/>
            <c:set var="businessObjectClass" value="${fn:replace(attributeEntry.control.businessObject,'.','|')}"/>
            <html:select styleId="${!empty propertyId ? propertyId : property}" property="${property}" title="${accessibleTitle}" tabindex="${tabindex}" style="${textStyle}" disabled="${disableField}" onblur="${onblur}" onchange="${onchange}" styleClass="${styleClass}">
              <c:choose>
                <c:when test="${not empty businessObjectClass}">
                  <c:set var="methodAndParms" value="actionFormUtilMap.getOptionsMap${Constants.ACTION_FORM_UTIL_MAP_METHOD_PARM_DELIMITER}${finderClass}${Constants.ACTION_FORM_UTIL_MAP_METHOD_PARM_DELIMITER}${businessObjectClass}${Constants.ACTION_FORM_UTIL_MAP_METHOD_PARM_DELIMITER}${attributeEntry.control.keyAttribute}${Constants.ACTION_FORM_UTIL_MAP_METHOD_PARM_DELIMITER}${attributeEntry.control.labelAttribute}${Constants.ACTION_FORM_UTIL_MAP_METHOD_PARM_DELIMITER}${attributeEntry.control.includeBlankRow}${Constants.ACTION_FORM_UTIL_MAP_METHOD_PARM_DELIMITER}${attributeEntry.control.includeKeyInLabel}"/>
    ...
    <c:when test="${attributeEntry.control.multiselect == true}">
            ...
            <c:if test="${disableField == false}"><input type="hidden" name="multiSelectToReset" value="${property}"/></c:if> 
	</c:when>
    <%-- radio --%>
    ...
    <c:when test="${attributeEntry.control.checkbox == true}">
            <html:checkbox property="${property}" style="${textStyle}" title="${accessibleTitle}" tabindex="${tabindex}" disabled="${disableField}" onblur="${onblur}"
            	...
            	styleClass="${styleClass}"/>
            <c:if test="${disableField == false}"><input type="hidden" name="checkboxToReset" value="${property}"/> </c:if>
    </c:when>
    <%-- hidden --%>
    <c:when test="${attributeEntry.control.hidden == true}">
		<c:if test="${!sessionDocument}"><html:hidden property="${property}" /></c:if>
    </c:when>
    <%-- currency --%>
    ...
    <c:otherwise>unknown control type for: ${attributeEntry.name}</c:otherwise>
  </c:choose>
  <%-- error icon --%>
  <c:if test="${hasErrors}"><kul:fieldShowErrorIcon /></c:if>
  <%-- datePicker icon --%>
