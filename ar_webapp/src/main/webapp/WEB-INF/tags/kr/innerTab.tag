...
/><c:choose>
    <c:when test="${useCurrentTabIndexAsKey}"><c:set var="tabKey" value="${currentTabIndex}" scope="request"/></c:when>
    <c:otherwise><c:set var="tabKey" value="${kfunc:generateTabKey(parentTab)}:${kfunc:generateTabKey(tabTitle)}" scope="request"/></c:otherwise>
</c:choose>
    <c:if test="${empty overrideDivClass}"><c:set var="overrideDivClass" value="innerTab-head"/></c:if>
...
<c:if test="${! empty tabItemCount}"><c:set var="tabTitle" value="${tabTitle} (${tabItemCount})" /></c:if>
...
<c:if test="${! (empty tabErrorKey)}"><div class="tab-container-error"><div class="left-errmsg-tab"><kul:errors keyMatch="${tabErrorKey}"/></div></div></c:if>
