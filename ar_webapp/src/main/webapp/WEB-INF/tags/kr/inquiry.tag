...
<c:if test="${render}"><a href="${ConfigProperties.application.url}/kr/inquiry.do?methodToCall=start&businessObjectClassName=${boClassName}&${keyValues}"  target="_blank"></c:if>
<jsp:doBody/>
<c:if test="${render}"></a></c:if>