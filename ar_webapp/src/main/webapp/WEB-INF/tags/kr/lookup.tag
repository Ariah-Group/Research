...
<c:choose>
  <c:when test="${!empty searchIconOverride}"><c:set var="lookupicon" value="${searchIconOverride}"/></c:when>
  <c:otherwise><c:set var="lookupicon" value="${ConfigProperties.kr.externalizable.images.url}searchicon.gif"/></c:otherwise>
</c:choose>
