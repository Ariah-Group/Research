...
<c:forTokens items='${mapName}' var='namePart' delims='.'>
    <c:choose><c:when test="${empty walker}">
            <c:set var='walker' value="${pageScope[namePart]}" />
            <c:if test="${empty walker}"><c:set var='walker' value="${requestScope[namePart]}" /></c:if>
            <c:if test="${empty walker}"><c:set var='walker' value="${sessionScope[namePart]}" /></c:if>
            <c:if test="${empty walker}"><c:set var='walker' value="${applicationScope[namePart]}" /></c:if>
        </c:when>
        <c:otherwise><c:set var='walker' value="${walker[namePart]}" /></c:otherwise>
    </c:choose></c:forTokens><c:set var='valueHolder' value="${walker}" />