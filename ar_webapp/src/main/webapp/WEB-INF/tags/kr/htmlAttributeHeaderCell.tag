...
%><%@ attribute name="attributeEntry" required="false" type="java.util.Map" description="The entry of an attribute in the DataDictionary to use to generate the label."
%><%@ attribute name="attributeEntryName" required="false" description="The full name of the DataDictionary entry to use as the attributeEntry. E.g., 'DataDictionary.Budget.attributes.budgetProjectDirectorUniversalIdentifier'."
%><%@ attribute name="literalLabel" required="false" description="A String to use for the label instead of the DataDictionary."
...
<th rowspan="${empty rowspan ? 1 : rowspan}" colspan="${empty colspan ? 1 : colspan}"
    ${empty scope ? '' : scopeAttribute}
    ${empty align ? (!horizontal ? '' : 'align="right"') : alignAttribute}
    ${empty width ? '' : widthAttribute}
    ${empty nowrap ? '' : 'nowrap'}
    >&nbsp;
    <c:if test="${not empty anchor}"><a name="${anchor}"></a></c:if>
...
>${literalLabel}<c:if test="${not empty labelFor}"></label></c:if>
        </c:when>
        <c:otherwise>
