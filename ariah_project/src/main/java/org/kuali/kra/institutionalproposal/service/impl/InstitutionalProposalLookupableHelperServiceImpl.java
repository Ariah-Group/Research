...
/**
 * This class is used to control behavior of Institutional Proposal lookups.
 * Depending on where the lookup is coming from, we may need to add custom
 * action links and/or filter the lookup results.
 */
public class InstitutionalProposalLookupableHelperServiceImpl extends KraLookupableHelperServiceImpl {
    ...
    /**
     * This method filters results so that the person doing the lookup only gets
     * back the documents he can view.
     *
     * @param results
     * @return
     */
    ...
    /**
     * This method is to filter out IP's not having codes in the valid funding
     * proposal status codes parameter.
     *
     */
    ...
    /**
     * Find if any proposal associate with this INSP has 'Approval Pending
     * Submitted' proposal state type
     *
     */
    protected boolean isDevelopmentProposalAppPendingSubmitted(InstitutionalProposal ip) {
        ...
        for (DevelopmentProposal developmentProposal : devProposals) {
            if (ProposalState.APPROVAL_PENDING_SUBMITTED.equals(developmentProposal.getProposalStateTypeCode())) {
