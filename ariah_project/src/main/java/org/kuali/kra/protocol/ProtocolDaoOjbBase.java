...
public abstract class ProtocolDaoOjbBase<GenericProtocol extends ProtocolBase> extends PlatformAwareDaoBaseOjb implements OjbCollectionAware, ProtocolDao<GenericProtocol> {
    ...
    private static final int FETCH_SIZE_NUM_RECORDS = 1000;
    ...
    public List<GenericProtocol> getProtocols(Map<String, String> fieldValues) {
        ...
        q.setFetchSize(FETCH_SIZE_NUM_RECORDS);
     ...
     * /*
     *
     * Builds up criteria object based on the object and map. This method is
     * copied from lookdaoojb, but not published in lookupdao, so can't access
     * it directly.
     *
    ...
    /**
     * select count(b.protocol_number) into li_SubmissionCount from osp$protocol
     * b where b.protocol_number like as_protocol_number || '%' and
     * b.sequence_number = (select max(a.sequence_number) from osp$protocol a
     * where a.protocol_number = b.protocol_number) and b.protocol_status_code
     * in (100, 101, 102, 103, 104, 105, 106);
     *
     * Replaced above query with following query by removing sub-query and added
     * java code to to get sub-query behaviour.
     *
     * select (b.protocol_number) from protocol b where b.protocol_number like
     * '001%' and b.protocol_status_code in (100, 101, 102, 103, 104, 105, 106);
     *
     * @see
     * org.kuali.kra.protocol.ProtocolDao#getProtocolSubmissionCount(java.lang.String)
     */
    @Override
    public boolean getProtocolSubmissionCountFromProtocol(String protocolNumber) {
        ...
        // small performance improvement
        query.setFetchSize(FETCH_SIZE_NUM_RECORDS);
