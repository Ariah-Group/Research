...
public class AwardTemplateSyncServiceImpl implements AwardTemplateSyncService {
    ...
    public boolean syncAwardToTemplate(AwardDocument awardDocument, AwardTemplateSyncScope[] scopes) {
        ...
        try {
            ...
            throw new RuntimeException(String
                    .format("Exception thrown executing syncAwardToTemplate on document %s with scopes {%s}", awardDocument.getDocumentNumber(), scopesArrayToDelimitedList(scopes)), e);
        }
        return success;
    }
    ...
    public boolean syncWillAlterData(AwardDocument awardDocument, AwardTemplateSyncScope scope) {
        ...
        try {
            ...
            throw new RuntimeException(String
                    .format("Exception thrown executing syncWillAlterData on document %s with scopes {%s}", awardDocument.getDocumentNumber(), scopesArrayToDelimitedList(scopes)), e);
        }
        return result;
    }
    ...
    public boolean templateContainsScopedData(AwardDocument awardDocument, AwardTemplateSyncScope scope) {
        ...
        try {
            ...
            throw new RuntimeException(String
                    .format("Exception thrown executing syncSourceCheck on document %s with scopes {%s}", awardDocument.getDocumentNumber(), scopesArrayToDelimitedList(scopes)), e);
        }
        return result;
    }
     ...
     * if scope = FULL, then all values of AwardTemplateSyncScope.values() is
     * used, otherwise an array with one element is returned.
     *
    ...
    protected AwardTemplateSyncScope[] getScopeArray(AwardTemplateSyncScope scope) {
        ...
        if (scope.equals(AwardTemplateSyncScope.FULL)) {
    ...
    /**
     * This method returns an empty scope stack. This stack is used to keep
     * track of the sync scopes of each object as we traverse the object graph.
     * The allows for the scope CONTAININIG_CLASS_INHERIT which for an element
     * or list that will match the scopes of the containing class.
     *
    ...
    protected boolean checkTargetField(Object awardTemplateObject, Object awardObject, Field field, java.util.Stack<AwardTemplateSyncScope[]> scopeStack, Award award, AwardTemplate awardTemplate) throws Exception {
        boolean result = false;
        if (field.isAnnotationPresent(AwardSyncable.class)) {
            if (ObjectUtils.getPropertyValue(awardObject, field.getName()) == null) {
                result = false;
            } else {
                result = true;
            }
    ...
    protected boolean checkSourceField(Object awardTemplateObject, Object awardObject, Field field,
        ...
        boolean result = false;
        if (ObjectUtils.isNotNull(ObjectUtils.getPropertyValue(awardTemplateObject, field.getName())) && field.getAnnotation(AwardSyncable.class).impactSourceScopeEmpty()) {
            result = true;
        }
        return result;
    }
    ...
    /**
     * This method is for extracting the appropriate list from award by using
     * property name and sync the list
     *
     * @param awardTemplateObject
     * @param awardObject
     * @param propertyName
    ...
    /**
     * This method calculates the scope that should be used for an element given
     * a scope stack. The rule is simple: Move down the stack until you reach an
     * array that contains something other that CONTAINING_CLASS_INHERIT. If no
     * such element exists in the stack we throw an exception.
     *
     * @param scopeStack The scope stack you want to get the effective scope
     * for.
     * @return An array representing the effective scope to be used.
     * @throws IllegalStateException if there is no array in the stack that
     * contains a scope other than CONTAINING_CLASS_INHERIT.
     */
    protected AwardTemplateSyncScope[] getEffectiveScope(Stack<AwardTemplateSyncScope[]> scopeStack)
        ...
        for (int i = scopeStack.size() - 1; i >= 0; i--) {
            if (ArrayUtils.contains(scopeStack.get(i), AwardTemplateSyncScope.CONTAINING_CLASS_INHERIT)) {
                continue;
            }
    ...
    /**
     *
     * This method is to invoke individual method if there is a method mentioned
     * in the AwardSyncableList annotation
     *
     * @param awardObject
    ...
    /**
     * This method is used to sync member properties of an award template object
     * to an award object
     *
     * @param awardTemplateObject
     * @param awardObject
    ...
    /**
     * This method uses recursion to find all declared fields for a class
     * hierierachy
     *
     * @param klass
     * @param allFields
    ...
    /**
     * Check the target list, returns true if there is an element in the
     * collection that is in scope.
     *
     * @param awardTemplateObject
     * @param awardObject
     * @param propertyName
    ...
    /**
     * This is the default method use to sync objects. This method is
     * implemented to work on row Object. If we need to change the
     * implementation of sync functionality for a specific list, we have to
     * overload this method with specific type.
     *
     * @param awardObject
     * @param listObject
     * @param objectInList
    ...
    /**
     * This method finds the appropriate getOrCreateNewListElementObject for a
     * class. It does this by first trying to return the method within this
     * implementation with the signature getOrCreateNewListElementObject(
     * syncSourceClass, Class.class, Award.class, AwardTemplate.class ). If it
     * does not find one, it returns the generic method
     * getOrCreateNewListElementObject( Object.class, Class.class, Award.class,
     * AwardTemplate.class ).
     *
     * @param syncSourceClass - The class you wish to find the method for. If a
     * specific method for this class does not exist, the generic method is
     * returned.
    ...
    /**
     * This method is to fetch the award template by using template code from
     * Award object.
     *
     * @param Award object
    ...
    /**
     * Utility method to generate a comma delimited list of scopes contained in
     * the array passed in.
     *
     * @param scopes
     * @return Comma delimited list of scope name contained in the passed in
     * scopes array.
     */
