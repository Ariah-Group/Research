...
public class AwardFandaRateRule extends ResearchDocumentRuleBase implements AddFandaRateRule {
    ...
    /**
     *
     * This is a convenience method for evaluating the rule for
     * applicableFandaRate field.
     *
    ...
    /**
     *
     * This is a convenience method for evaluating the rule for fiscalYear
     * field.
     *
     * @param awardFandaRate
     * @param propertyPrefix
    ...
    protected boolean evaluateRuleForFiscalYear(AwardFandaRate awardFandaRate, String propertyPrefix) {
        ...
        if (awardFandaRate.getFiscalYear() == null
            ...
            rulePassed = false;
            reportError(propertyPrefix + FISCAL_YEAR_STRING, KeyConstants.ERROR_REQUIRED_FISCAL_YEAR);
        } else if (awardFandaRate.getFiscalYear().length() != FISCAL_YEAR_LENGTH) {
            rulePassed = false;
            reportError(propertyPrefix + FISCAL_YEAR_STRING, KeyConstants.ERROR_FISCAL_YEAR_INCORRECT_FORMAT, "Fiscal Year");
        } else {
            try {
                Integer.parseInt(awardFandaRate.getFiscalYear());
            } catch (NumberFormatException e) {
                rulePassed = false;
                reportError(propertyPrefix + FISCAL_YEAR_STRING, KeyConstants.ERROR_FISCAL_YEAR_INCORRECT_FORMAT);
            }
        }
        return rulePassed;
    }
    ...
    /**
     *
     * This is a convenience method for evaluating the rule for startDate and
     * endDate fields.
     *
     * @param awardFandaRate
     * @param propertyPrefix
    ...
    protected boolean evaluateRuleForStartAndEndDates(AwardFandaRate awardFandaRate, String propertyPrefix) {
        ...
        if (!rule2Passed) {
            reportError(propertyPrefix + ".endDate", KeyConstants.ERROR_END_DATE_BEFORE_START_DATE_INDIRECT_COST_RATE, DATE_PARAMS);
        }
    ...
    /**
     *
     * This method checks whether FandaRares are available in valid Rate table
     * or not;
     *
     */
    ...
    /**
     * This method returns the valid rates from valid rates table that match the
     * rates attached to the award.
     *
     * @return
     */
    ...
    private Collection<ValidRates> getValidRatesForFandA(String rateTypeOnOrOff, KualiDecimal rate) {
        ...
        return (Collection<ValidRates>) getKraBusinessObjectService().findMatching(ValidRates.class, rateValues);
    }
     ...
     * iterates through it twice to find out whether both on campus and off
     * campus entries are present for every indirectRateTypeCode. Returns true
    ...
    protected boolean isFandaRateInputInPairs(List<AwardFandaRate> awardFandaRateList) {
        ...
        if (!valid) {
            GlobalVariables.getMessageMap().putError(FANDA_RATES, KeyConstants.INDIRECT_COST_RATE_NOT_IN_PAIR);
        }
        isPairChecked = true;
        if (valid) {
            int i = 0;
            for (AwardFandaRate awardFandaRate1 : awardFandaRateList) {
                if (StringUtils.equalsIgnoreCase(awardFandaRate1.getOnCampusFlag(), Constants.ON_CAMPUS_FLAG)) {
                    int j = 0;
                    if (getValidRatesForFandA(ON_CAMPUS_RATE, awardFandaRate1.getApplicableFandaRate()).size() > 0) {
                        for (AwardFandaRate awardFandaRate2 : awardFandaRateList) {
                            if (StringUtils.equalsIgnoreCase(awardFandaRate2.getOnCampusFlag(), Constants.OFF_CAMPUS_FLAG)
    ...
    /**
     *
     * This method iterates through the awardFandaRateList and creates two
     * hashmaps; one with on campus values and other with off campus values in
     * it.
     *
     * @param awardFandaRateList
     * @param a1
    ...
    protected void createHashMapsForRuleEvaluation(List<AwardFandaRate> awardFandaRateList,
        ...
        for (AwardFandaRate awardFandaRate : awardFandaRateList) {
            if (StringUtils.equalsIgnoreCase(awardFandaRate.getOnCampusFlag(), Constants.ON_CAMPUS_FLAG)) {
                a1.put(awardFandaRate.getFandaRateTypeCode(), awardFandaRate.getOnCampusFlag());
            } else if (StringUtils.equalsIgnoreCase(awardFandaRate.getOnCampusFlag(), Constants.OFF_CAMPUS_FLAG)) {
